...
## Finally, adding the follow code to obtain the results in a data frame
...
xx <- c(22.1, 30.4, 55, 1, 2, 10, 3, 5, 9, 10)
yy <- fizzbuzz(x = xx)

xx <-c (-1, -2, -3, 10, 15, 9, -3, 0, 12, 15)
yy <- fizzbuzz(x = xx)
...
Discussion: \
The order of each statement of 'stopifnot' is important in the above code. I followed the questions below to check input validation:
1. Are inputs numeric?
2. Are inputs integers (Inputs are sampled from 1:100)?
3. Are inputs positive?
4. Are input has same length?
