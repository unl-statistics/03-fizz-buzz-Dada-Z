...
fizzbuzz <- function(x) {
  ...
  stopifnot(all(x)>=1)
  ## check the input and output have same length
  ##check length/index same
  stopifnot(length(yy) == length(x))
...
## Check multiple inputs: x=22.2, x="az"
yy <- fizzbuzz(x = xx)
...
```{python}
xx = [1, 2, 3, 4, 5]  # This is a list, not a numpy array
yy = fizzbuzz(x=xx)
